...
export const FetchData =
  (caller: string, page: number) => async (dispatch: Dispatch<DispatchObj>) => {
    const response = await axios.get(
      `https://rickandmortyapi.com/api/${caller}?page=${page}`
    );
    dispatch({ type: PickleTypes.fetchData, payload: response.data });
  };

export const AddTodo =
  (editValues: Todo[]) => async (dispatch: Dispatch<DispatchObj>) => {
    const response = await axios.patch(`http://localhost:3001/storage/`, {
      todos: editValues,
    });
...
export const DeleteTodo =
  (todos: Todo[], todo: Todo) => async (dispatch: Dispatch<DispatchObj>) => {
    const response = await axios.patch(`http://localhost:3001/storage/`, {
